...
struct MenuAction {
  ...
  int  item = -1;
  bool play = false;
  bool opCast(T : bool)() const { return menu >= 0 && item >= 0 || play; }
}
...
MenuAction drawTopBar(Rectangle r, string sceneName) {
  ...
  if (activeMenu >= 0) {
    ...
    foreach (j, ref item; MENUS[mi].items) {
      if (GuiButton(Rectangle(dx, iy, DROPDOWN_W, ITEM_H), item.label.toStringz)) {
        result     = MenuAction(-1, -1, false); // clear play before overwriting
